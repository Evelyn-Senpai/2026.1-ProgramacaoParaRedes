'''Servidor de arquivos http TCP com Threads'''

import sys # Importe do sys para chamar o arquivo na linha de comando.
import threading # Importe do threading para criar threads.
...
def threads(site, resource, output): # Função para criar threads.
    threads_list = [] # Lista de threads.
    while True: # Enquanto for verdade.
        thread = threading.Thread(target=get_data, args=(site, resource, output)) # Criação da thread.
        
        threads_list.append(thread) # Adiciona a thread na lista.
        
        thread.start() # Inicia a thread.

        for thread in threads_list: # Para cada thread na lista.
            thread.join() # Espera a thread terminar.

if len(sys.argv) == 4: # Se houver 4 argumentos na linha de comando.
    threads(sys.argv[1], sys.argv[2], sys.argv[3]) # A função é chamada com esses quatro argumentos.
